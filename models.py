...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        posteriors = []
        for sentence in parallel_corpus:
            posterior = self.translation_probs[sentence.source_tokens[:, None], sentence.target_tokens]
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            params_probs = self.translation_probs[
                sentence.source_tokens[:, None],
                sentence.target_tokens
            ]

            elbo -= sentence.target_tokens.shape[0] * np.log(sentence.source_tokens.shape[0])
            elbo -= np.dot(posterior.ravel(), self._log(posterior).ravel())
            elbo += np.dot(posterior.ravel(), self._log(params_probs).ravel())
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            np.add.at(self.translation_probs, (sentence.source_tokens[:, None], sentence.target_tokens), posterior)
        self.translation_probs /= self.translation_probs.sum(axis=1)[:, None]
    ...
    def align(self, sentences: List[TokenizedSentencePair]):
        posteriors = self._e_step(sentences)
        result = []
        for sentence, posterior in zip(sentences, posteriors):
            args = posterior.argmax(axis=0)
            alignment = [(i + 1, j + 1) for j, i in enumerate(args, 0)]
            result.append(alignment)
        return result
...
class WordPositionAligner(WordAligner):
    ...
    def _get_probs_for_lengths(self, src_length: int, tgt_length: int):
        ...
        if key not in self.alignment_probs:
            self.alignment_probs[key] = np.full(key, 1.0 / src_length)
    ...
    def _e_step(self, parallel_corpus):
        ...
        for sentence in parallel_corpus:
            alignment_probs = self._get_probs_for_lengths(
                sentence.source_tokens.shape[0],
                sentence.target_tokens.shape[0]
            )
            posterior = self.translation_probs[
                            sentence.source_tokens[:, None],
                            sentence.target_tokens
                        ] * alignment_probs
            posterior /= posterior.sum(axis=0)[None, :]
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            ...
            elbo += np.dot(posterior.ravel(), self._log(alignment_probs).ravel())
            elbo -= np.dot(posterior.ravel(), self._log(posterior).ravel())
            elbo += np.dot(posterior.ravel(), self._log(params_probs).ravel())
        return elbo
    ...
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for sentence in parallel_corpus:
            ...
            alignment_probs /= alignment_probs.sum(axis=0)[None, :]
